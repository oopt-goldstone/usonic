...
FROM debian:bullseye AS sairedis
...
FROM debian:bullseye AS libteam
...
FROM debian:bullseye AS lldpd

RUN --mount=type=cache,target=/var/cache/apt --mount=type=cache,target=/var/lib/apt,sharing=locked  \
apt update && apt install -qy --no-install-recommends make g++ graphviz autotools-dev autoconf doxygen stgit libnl-genl-3-dev libnl-nf-3-dev libhiredis-dev perl libxml-simple-perl aspell swig libgtest-dev dh-exec debhelper libtool python3-all python-all libpython3-dev libpython2-dev quilt patchelf libboost-dev libbsd-dev pkg-config check libsnmp-dev libpci-dev libxml2-dev libevent-dev libreadline-dev libcap-dev libjansson-dev python3 python python3-pip build-essential python3-setuptools wget
...
FROM debian:bullseye AS sonic-frr
...
RUN --mount=type=bind,source=./sm/sonic-frr,target=/root/sm/sonic-frr,rw \
    --mount=type=bind,source=./make/sonic-frr,target=/root/make/sonic-frr,rw \
    cd /root && make -C /root/make/sonic-frr
...
RUN --mount=type=cache,target=/var/cache/apt --mount=type=cache,target=/var/lib/apt,sharing=locked  \
apt update && apt-get install -qy -y --no-install-recommends apt-utils libpython2-dev strace vim gdb procps redis-server syslog-ng tcpdump libdaemon-dev libdbus-1-dev libjansson-dev  pkg-config debhelper libdbus-1-3 libdaemon0 libjansson4 libc-ares2 iproute2 libpython2.7 libjson-c-dev logrotate libunwind8 python3 python python3-pip libbsd-dev check libsnmp-dev libpci-dev libxml2-dev libevent-dev libreadline-dev libcap-dev build-essential libboost-dev libzmq5-dev
